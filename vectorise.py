...
class VectorIndex():
    ...
    #build two databases linked by unique id
    def create_db(self, resume=None, until=None):

        data = pd.read_csv('scripts_wrapped.csv')

        # using npy instead of the original csv
        batch_size = 100000
        for x in range((len(data) // batch_size) + 1):

            # handle resume & until ##############
            if resume is not None and x < resume:
                continue
            ...
            _from = batch_size * x
            _to = batch_size * (x + 1)
            if batch_size * (x + 1) > len(data):
                _to = len(data)
            id_batch = np.array(data['video_id'][_from:_to], dtype='S')
            text_batch = np.array(data['text'][_from:_to])
            timestamp_batch = np.array(data['timestamp'][_from:_to], dtype='S')

            # vectorise script using SentenceBERT
            start = time.time()
            log.info('starting to vectorise data batch [{}:{}]'.format(_from, _to))

            # # # run to test invalid inputs in text ###################################################
            # bob_size = 1
            # for i in range(len(text_batch) // bob_size):
            #     log.info('finding invalid sentences in batch no. {}/{}!'.format(i, len(text_batch) // bob_size))
            #     # divide data into batches of million entries:
            #     bfrom = bob_size * x
            #     bto = bob_size * (x + 1)
            #     bob = text_batch[bfrom:bto]
            #     try:
            #         text_vectorised = self.model.encode(bob)
            #     except:
            #         log.info('================ conversion failed! check this out ================')
            #         log.info('invalid sentences in  batch no.{}'.format(i))
            #         log.info(bob)
            #         raise ValueError('conversion failed! FFS')
            # # #######################################################################################

            text_batch_vectorised = self.vh.model.encode(sentences=text_batch, show_progress_bar=True)
            log.info('{} sentences vectorised in {} seconds!'.format(len(text_batch), round(time.time() - start, 2)))

            start = time.time()

            vector_ids = self.generate_ids(size=batch_size,
                                           batch_count=x)

            # key: vector_id, value: all other info (video id, timestamp and text)
            id2info = SqliteDict(self.vh.id2info_path, autocommit=True)

            # key vector_id, value: vectorised sentence
            id2vector = SqliteDict(self.vh.id2vector_path, autocommit=True)

            for i in range(_to - _from):
                if i % 1000 == 0:
                    # print out progress status ##############################################################
                    sys.stdout.write('writing vectorised data to database ... {}/{}'.format(i, _to - _from))
                    sys.stdout.flush()
                    time.sleep(0.1)
                    ##########################################################################################

                id2info[vector_ids[i]]= {'video_id':id_batch[i],
                                         'timestamp':timestamp_batch[i],
                                         'text':text_batch[i]
                                         }
                id2vector[vector_ids[i]] = text_batch_vectorised[i]

            id2info.close()
            id2vector.close()

            log.info('\nsuccessfully reformatted the results in {} seconds!'.format(time.time() - start))

    def are_same_vectors(self, v1, v2):
        return self.vh.distance(v1,v2) < 1e-4

    #checks if the vectors in database match freshly encoded vectors
    def vectorised_data_test(self, test_type, mode=None, resume=None, size=100):
        if test_type == 'key':
            if mode == 'vector':
                path = self.vh.id2vector_path
            elif mode == 'info':
                path = self.vh.id2info_path
            else:
                raise ValueError('wrong mode entered! mode should be either "vector" or "info".')

            log.info('loading database id2{}!'.format(mode))
            database = SqliteDict(path, autocommit=True)
            true_keys = []
            saved_keys = []
            start = time.time()

            #### generate keys from numbers as we did in create_db() to get a list of "true" keys ##########################
            for i in range(self.vh.num_items):
                true_keys.append(self.vh.hasher.encode(i))
                if i % 10000 == 0:
                    sys.stdout.write('\r')
                    sys.stdout.write('creating true keys ... {}/{} running time: {} seconds'.format(i, self.vh.num_items, round(time.time()-start,4)))
                    sys.stdout.flush()
                    time.sleep(0.01)
            start = time.time()
            log.info('\nsorting true_keys!')
            true_keys.sort()
            log.info('sort finished in {} seconds'.format(time.time() - start))
            ################################################################################################################

            #### get a list of keys saved in the database ##################################################################
            counter = 0
            for key in database.keys():
                saved_keys.append(key)
                if counter % 10000 == 0:
                    sys.stdout.write('\r')
                    sys.stdout.write('retrieving saved keys ... {}/{} running time: {} seconds'.format(counter, self.vh.num_items, round(time.time() - start,4)))
                    sys.stdout.flush()
                    time.sleep(0.01)
                counter += 1
            log.info('\nsorting saved_keys!')
            ...
            #### compare keys from each list to test the keys ##############################################################
            counter = 0
            for true, saved in zip(true_keys, saved_keys):
                if counter % 10000 == 0:
                    sys.stdout.write('\r')
                    sys.stdout.write('validating keys ... {}/{} running time: {} seconds'.format(counter, self.vh.num_items, round(time.time() - start,4)))
                    sys.stdout.flush()
                    time.sleep(0.01)
                counter += 1
                if true != saved:
                    log.info('true: {}, saved: {} unmatching keys!'.format(true, saved))
            ################################################################################################################

            log.info('\nid2{} key test finished!'.format(mode))

            database.close()
            log.info('database closed successfully!')

        elif test_type == 'vectorisation':
            start = time.time()
            log.info('start testing ...')
            _from = 0
            _to = self.vh.num_items
            randnums = [randrange(_from, _to) for iter in range(size)]

            original_text_data = pd.read_csv('scripts_wrapped.csv')['text']
            id2vector = SqliteDict(self.vh.id2vector_path, autocommit=True)
            id2info = SqliteDict(self.vh.id2info_path, autocommit=True)

            for i, num in enumerate(randnums):
                test_key = self.vh.hasher.encode(num)
                test_text = original_text_data[num]

                correct_v = self.vh.model.encode(test_text)
                retrieved_v = id2vector[test_key]
                retrieved_text = id2info[test_key]['text']

                vector_test_status = 'Fail'
                if self.are_same_vectors(correct_v, retrieved_v):
                    vector_test_status = 'Success'

                text_test_status = 'Fail'
                if len(set(list(test_text))) == len(set(list(retrieved_text))):
                    text_test_status = 'Success'

                if vector_test_status == 'Fail' or text_test_status == 'Fail':
                    log.info('\nTest has failed at key [{}] and test text [{}]'.format(test_key, test_text))
                    log.info('instead the system got [{}] from key [{}]'.format(retrieved_text, test_key))

                sys.stdout.write('\r')
                sys.stdout.write('test result - vectorisation: [{}], text_retrieval: [{}] | progress: {}/{} | running time: {} sec'.
                                 format(vector_test_status, text_test_status, i, size, round(time.time() - start,4)))
                sys.stdout.flush()
                time.sleep(0.01)
    ...
    def vectorise_example(self):
        # data = pd.read_csv('scripts.csv')
        # sample_text = 'One year ago, I rented a car in Jerusalem'

        sample_text = "I want to buy a car in Jerusalem"

        with open('sample_queries.txt', 'r') as f:
            sample_queries = f.read().split('\n')
            try:
                sample_queries.remove('')
            except:
                pass

        query_vectors = []
        for i, query in enumerate(sample_queries):
            query_vectors.append(self.vh.model.encode(query))


        sample_vector = self.vh.model.encode(sample_text)

        log.info('distance to self: {}'.format(self.vh.distance(sample_vector, sample_vector)))

        distances = {}
        start = time.time()
        for i, vector in enumerate(query_vectors):
            distances[sample_queries[i]] = self.vh.distance(sample_vector, vector)
        end = time.time()

        with open('sample_distance.csv', 'w') as f:
            f.write('"query","distance"\n')
            for key in result_sorted.keys():
                f.write('{},{}\n'.format(key, result_sorted[key]))

        data = pd.read_csv('sample_distance.csv')
        log.info(data)
        log.info('took {} seconds to calculate distance!'.format(end - start))

    def search_query(self, query):
        log.info('<============================================= calling function search_query =============================================>')
        start = time.time()
        ...
        start = time.time()
        ids = map(self.vh.hasher.encode, list(range(limit)))
        log.info('ids encoded in {} seconds!'.format(time.time()-start))

        start = time.time()
        closest_d = 100
        closest_text = ''
        for i, id in enumerate(ids):
            distance = self.vh.distance(encoded_query, id2vector[id])
            if distance < closest_d:
                closest_d = distance
                closest_text = id2info[id]['text']

        log.info('search result: \ndistance: {},\ntext: {}\nsearch cost:{} sec'.format(closest_d, closest_text, round(time.time()-start,4)))
    ...
    def search_query_fast(self, query, n_best, distance_threshold):
        ...
        for cluster in clustered_ids.iterkeys():
            ...
            distance = self.vh.distance(encoded_query, clustered_ids[cluster]['centroid'])
        ...
        log.info('found closest cluster in {} seconds!'.format(time.time()-start))
        ...
        for id in clustered_ids[closest_cluster]['members']:
            ...
            if distance < vector_d_threshold:
                ...
                result_texts[str(distance)] = {'video_id': id2info[id]['video_id'],
                                               'timestamp':'{}:{}'.format(int(float(id2info[id]['timestamp']) // 60),
        ...
        id2vector.close(force=True)
        id2info.close(force=True)
        clustered_ids.close(force=True)
...
class DatabaseHandler():
    ...
    def ensembled_clustering(self, n_clusters, n_components):
        log.info('<=================================== running ensembled_clustering ===================================>')
        id2vector = SqliteDict(self.vh.id2vector_path, autocommit=True)
        vectors = []
        num_items = self.vh.num_items
        chunk_size = 100000
        total_ids = list(range(num_items))
        kmeans = KMeans(init='k-means++', n_clusters=n_clusters, random_state=153)
        pca = PCA(n_components=n_components)
        partial_clustering_results = {}
        ...
        for i in range((num_items // chunk_size)+1):       #get indices to encode and retrieve vector from
            ...
            chunk = total_ids[i * chunk_size: (i+1)*chunk_size]
            log.info('working on ids {}:{}!'.format(i * chunk_size, (i+1)*chunk_size))

            for ctr, j in enumerate(chunk):                                 # append vectors into a list
                if ctr % 10000 == 0:
                    log.info('getting corresponding vectors: {}/{} running time: {} seconds'.format(ctr, chunk_size, round(time.time() - start, 4)))
                id = self.vh.hasher.encode(j)
                vectors.append(id2vector[id])

            # do PCA on batch data
            start = time.time()
            vectors_pcaed = pca.fit_transform(vectors)
            log.info('PCA fit and transformation on batch data no. {} finished in {} seconds!'.format(i, time.time() - start))

            # do kmeans on a slice of data
            start = time.time()
            labels = kmeans.fit_predict(vectors_pcaed)
            log.info('Clustering on batch data no.{} finished in {} seconds!'.format(i, time.time() - start))

            # get silhouette score to figure if kmeans is working well
            # sil = silhouette_score(vectors, kmeans.labels_)
            # log.info('silhouette score: {}'.format(sil))
            partial_clustering_results['batch_'+str(i)] = labels

            log.info('loop duration: {} seconds'.format(time.time()-loop_start))
            gc.collect()

        labels = []
        #now ensemble kmeans to get ensembled labels
        for key in partial_clustering_results.keys():
            labels.append(partial_clustering_results[key])
        labels = np.asarray(labels)
        log.info(labels.shape)
        # labels_ensembled = CE.cluster_ensembles(labels)

        #### save model to pickle
        with open('/media/chanwoo/CChoi/Work/year_5/ttds3_script_vectors/minibatch_kmeans.pkl', 'wb') as f:
    ...
    def incremental_fit_kmeans_model(self, n_clusters):
        ...
        for i in range((num_items // chunk_size) + 1):  # get indices to encode and retrieve vector from
            ...
            start = time.time()
            ids = map(self.vh.hasher.encode,chunk)
            vectors = list(map(id2vector.get,ids))
            ...
            log.info(
                'batched kmeans partial fit on batch data no. {} finished in {} seconds!'.format(i, time.time() - start))
    ...
    def save_mkmeans_data(self):
        ...
        for i in range((num_items // chunk_size) + 1):  # get indices to encode and retrieve vector from
            ...
            start = time.time()
            ids = list(map(self.vh.hasher.encode, chunk))
            vectors = list(map(id2vector.get,ids))
            log.info('mapped and converted vectors in {} seconds!'.format(time.time()-start))
...
def search_query_firestore(encoded_query, distance_threshold=3.4, n_best=10):
...
def main():
    ...
    vh = VariableHandler(
        ...
        clustered_ids_path='/media/chanwoo/CChoi/Work/year_5/ttds3_script_vectors/clustered_ids.sqlite',
        mkmeans_model_path='/media/chanwoo/CChoi/Work/year_5/ttds3_script_vectors/mkmeans_model.pkl',
        num_items=40356731,
        hasher=Hashids(salt='eerotke hameon jeoldaemorugetzi', min_length=7)
    )

    # vi = VectorIndex(vh)
    # vi.vectorise_example()
    # vi.create_vector_dictionary(9)
    # vi.load_vectorised_data()
    # vi.test()
    # vi.stringify_text('sample_scripts.csv')
    # vi.create_db(resume=93, until=100)
    ...
    # query = 'linked list is good'
    # vi.search_query_fast(query=query,
    #                      n_best=10,
    #                      distance_threshold=3.3)

    dh = DatabaseHandler(vh)
    query = 'good developer must be'

    """""""""incremental kmeans"""""""""
    # dh.ensembled_clustering(n_clusters=1000, n_components=10)
    # dh.incremental_fit_kmeans_model(1000)
